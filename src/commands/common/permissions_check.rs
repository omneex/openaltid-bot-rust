...
pub async fn check_if_mod(ctx: &Context, command: &ApplicationCommandInteraction, mongo_client: &Client) -> Result<bool, &'static str> {

    // Check if the user is an admin, admins always have permission.
    match &command.member {
        None => {}
        Some(mem) => {
            match mem.permissions {
                None => {}
                Some(perms) => {
...
pub async fn check_if_mod_comp(ctx: &Context, command: &MessageComponentInteraction, mongo_client: &Client) -> Result<bool, &'static str> {
