...
pub async fn command(ctx: &Context, command: &ApplicationCommandInteraction, mongo_client: &Client) {
    ...
    // If not allowed then send an error message to the user and return.
    if !allowed {
        interaction_error("You do not have permission to use this command.", &command, &ctx).await;
        return;
    }
    ...
    let role = match role_opt {
        None => {
            super::super::common::interaction_error::interaction_error("No role provided.", command, ctx).await;
            return;
        }
        Some(role) => role
    };
...
pub async fn register(ctx: &Context) {
    let result = ApplicationCommand::create_global_application_command(&*ctx.http, |command| {
        command
            ...
            .description("Set the verification role, this role is given when verified. Mod only command.")
            .create_option(|opt| {
                opt.name("role")
