...
use tracing::*;
use mongodb::*;
use crate::commands::common::{interaction_error::{interaction_error, interaction_error_comp}, permissions_check::check_if_mod_comp};
...
pub async fn command(ctx: &Context, command: &ApplicationCommandInteraction, mongo_client: &Client) {
    ...
    let delete_res = collection.delete_many(
        doc! {
            "user_ID": &user_id,
                            ...
                            components.create_action_row(|row| {
                                row.create_button(|button| {
                                    button
                                        ...
                                        .label("UNDO")
                                        .custom_id(format!("UndoRemoveConnection:{}:{}:{}", &user_id, &account_type, &account_id))
                                })
                            })
                        })
                })
...
pub async fn undo_callback(ctx: &Context, interaction: &MessageComponentInteraction, mongo_client: &mongodb::Client) {
    ...
    let delete_res = account_collection.insert_one(
        doc! {
            "user_ID": &user_id,
            "account_type": &account_type,
            "account_id": &account_id,
